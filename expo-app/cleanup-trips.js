...
async function deleteAllTrips() {
  ...
  // First delete from trip_members (foreign key constraint)
  const { error: membersError } = await supabase
    .from('trip_members')
    .delete()
    .neq('trip_id', '00000000-0000-0000-0000-000000000000'); // Delete all

  if (membersError) {
    console.error('Error deleting trip members:', membersError);
    return false;
  }
...
if (action === 'delete') {
  ...
  await listTrips();
  console.log('\nTo delete all trips, run: node cleanup-trips.js delete');
}
