 ...
 *
 * Run with: npx tsx test-prosemirror-html.ts
 */
...
const hasMarks1 = paragraph1?.content?.some((node: any) => node.marks && node.marks.length > 0);
console.log('✓ Has geo-marks:', hasMarks1);

if (hasMarks1) {
  const geoMarkNode = paragraph1.content.find((node: any) => node.marks && node.marks.length > 0);
...
const geoMarkCount = paragraph2?.content?.filter((node: any) => node.marks && node.marks.length > 0).length || 0;
...
const hasMarks3 = paragraph3?.content?.some((node: any) => node.marks && node.marks.length > 0);
...
const hasBoldMarks = paragraph4?.content?.some((node: any) =>
...
const hasHeading = result5.content?.some((node: any) => node.type === 'heading');
const hasParagraph = result5.content?.some((node: any) => node.type === 'paragraph');
console.log('✓ Has heading:', hasHeading);
console.log('✓ Has paragraph:', hasParagraph);

const paragraph5 = result5.content?.find((node: any) => node.type === 'paragraph');
const hasMarks5 = paragraph5?.content?.some((node: any) => node.marks && node.marks.length > 0);
