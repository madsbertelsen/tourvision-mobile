...
export default function GeoEdit() {
  ...
  const { locations, currentDoc, setGeoMarkUpdate } = useDocumentNextContext();
  ...
  const handleSave = () => {
    console.log('[GeoEdit] ========== handleSave called ==========');
    console.log('[GeoEdit] geoId:', geoId);
    console.log('[GeoEdit] selectedMode:', selectedMode);
    console.log('[GeoEdit] originLocation:', originLocation);
    console.log('[GeoEdit] routeGeometry:', routeGeometry);
    ...
    if (originLocation) {
      ...
      console.log('[GeoEdit] Found originGeoMark:', originGeoMark);
    ...
    if (routeGeometry && routeGeometry.coordinates) {
      ...
      console.log('[GeoEdit] Extracted waypoints, count:', updatedAttrs.waypoints.length);
    } else {
      updatedAttrs.waypoints = null;
    }

    console.log('[GeoEdit] Final updatedAttrs:', JSON.stringify(updatedAttrs, null, 2));

    // Send update to context
    console.log('[GeoEdit] Calling setGeoMarkUpdate with:', { geoId, updatedAttrs });
    setGeoMarkUpdate({
      geoId,
      updatedAttrs,
    });
    console.log('[GeoEdit] setGeoMarkUpdate called successfully');

    // Close the modal
    console.log('[GeoEdit] Calling router.back()');
    router.back();
  };
