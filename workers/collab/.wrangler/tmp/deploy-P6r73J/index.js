...
var YjsRoom = class {
  ...
  constructor(state, env) {
    ...
    this.ydoc.on("update", (update, origin) => {
      ...
      this.saveTimeout = setTimeout(() => this.persistState(), 2e3);
    });
   ...
   */
  handleSyncMessage(decoder, sender) {
    const syncMessageType = readSyncMessage(decoder, this.ydoc, sender);
    if (syncMessageType === messageYjsUpdate) {
      const encoder = createEncoder();
      writeVarUint(encoder, MESSAGE_SYNC);
      writeUpdate(encoder, readVarUint8Array(decoder));
      const message = toUint8Array(encoder);
   ...
   */
  handleAwarenessMessage(decoder, sender) {
    const update = readVarUint8Array(decoder);
    applyAwarenessUpdate(this.awareness, update, sender);
   ...
   */
  broadcastAwareness(update, exclude = null) {
    const encoder = createEncoder();
  ...
  async loadState() {
    try {
      ...
      if (stored && stored.length > 0) {
        ...
        applyUpdate(this.ydoc, state, "storage-load");
  ...
  async persistState() {
    try {
      const state = encodeStateAsUpdate(this.ydoc);
...
var src_default = {
  async fetch(request, env) {
    ...
    const id2 = env.YJS_ROOM.idFromName(documentId);
    const stub = env.YJS_ROOM.get(id2);
