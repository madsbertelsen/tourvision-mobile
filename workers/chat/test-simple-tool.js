...
/**
 * Test script for verifying basic tool calling works
 * This replicates the exact example from Cloudflare docs
 */
...
const DOCUMENT_ID = `test-simple-tool-${Date.now()}`;
    ...
    case 'ai_chunk':
      if (message.done) {
        ...
        if (!toolRequestReceived) {
          ...
          console.log('This suggests function calling is NOT working.');
        } else {
...
ws.on('close', (code, reason) => {
  ...
  if (!toolRequestReceived) {
    console.log('\n❗ Function calling is NOT working.');
    console.log('   Expected to see tool_request message for weather tool.');
  }
