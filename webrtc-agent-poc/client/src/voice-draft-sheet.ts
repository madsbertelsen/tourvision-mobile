...
// Auto-execute mode: Skip approval UI and execute plans immediately
let autoExecuteMode = true; // Set to true for auto-execution
...
export function showVoiceDraftSheet(transcript: string): void {
  ...
  analyzeWithAgent(transcript, getDocumentContext()).then(agentPlan => {
    ...
    // AUTO-EXECUTE: Skip UI, execute immediately
    if (autoExecuteMode && agentPlan) {
      executeAndNotify(agentPlan, transcript);
    } else {
      // Show approval UI (original behavior)
      const backdrop = document.getElementById('voice-draft-backdrop');
      const sheet = document.getElementById('voice-draft-sheet');
    ...
    // Handle case where no plan was generated
    if (!agentPlan) {
      console.warn('[VoiceDraftSheet] No plan generated by agent');
      if (autoExecuteMode) {
        showErrorNotification('Could not understand voice input. Please try again.');
      }
    }
  });
}
...
/**
 * Generate success message explaining what actions were taken
 */
function generateSuccessMessage(
  ...
  // Build message explaining what was done
  let message = `Understood: "${transcript}"\n\n`;
...
/**
 * Execute plan automatically without showing approval UI
 */
async function executeAndNotify(
  plan: AgentPlan,
  transcript: string
): Promise<void> {
  console.log('[VoiceDraftSheet] Auto-executing plan');

  if (!editorView || !schema) {
    console.error('[VoiceDraftSheet] EditorView or Schema not available');
    showErrorNotification('Failed to execute: Editor not ready');
    return;
  }
  ...
  try {
    ...
    if (allSuccess) {
      console.log('[VoiceDraftSheet] ✅ Plan executed successfully');
      plan.status = 'complete';

      // Generate success message
      const message = generateSuccessMessage(transcript, plan, results);
      ...
      console.error('[VoiceDraftSheet] ❌ Execution failed:', errors);
      plan.status = 'failed';
      plan.error = errors;

      showErrorNotification(`Failed to execute: ${errors}`);
    }
  } catch (error: any) {
    console.error('[VoiceDraftSheet] Execution error:', error);
...
/**
 * Show success notification after auto-executing plan
 */
function showSuccessNotification(message: string): void {
  console.log('[VoiceDraftSheet] Success:', message);
  showTemporaryMessage(message, 'success');
}

/**
 * Show error notification if plan execution fails
 */
function showErrorNotification(message: string): void {
  console.error('[VoiceDraftSheet] Error:', message);
 ...
 */
function showTemporaryMessage(message: string, type: 'success' | 'error'): void {
  ...
  // Remove after 5 seconds (increased from 3 to allow reading)
  setTimeout(() => {
